...
(async () => {
  ...
  app.get('/authorize', (req, res, next) => {
   const authorizeUrl = client.authorizationUrl({
      redirect_uri: 'http://localhost:5000/okta/callback',
      scope: 'openid email',
      state: '1234567890',
      response_type: 'code'
    });
  ...
  app.get('/okta/callback', async (req, res, next) => {
    if (req.query.error) {
      ...
      const tokenSet = await client.authorizationCallback('http://localhost:5000/okta/callback', req.query, {state: '1234567890'})
      res.send(tokenSet)
    }
  })
